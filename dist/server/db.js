...
import { dailySnapshots, statusChanges } from '../drizzle/schema.js';
...
export async function getSnapshotHistory(days = 30) {
    ...
    const dataMap = new Map(snapshots.map(s => {
 ...
 * Track status change
 */
export async function trackStatusChange(data) {
 ...
 */
export async function getFieldChanges(fieldName, limit = 10) {
    const results = await db
        .select()
        .from(statusChanges)
